...
export function buildConstructionFunding(project: ProjectModel, periods: Period[]) {
  ...
  const totalCapex = project.construction.capex * (scenario?.capexMultiplier ?? 1) + project.construction.contingency + project.construction.costOverrunReserve + (project.construction.includeVatBridge === false ? 0 : project.construction.vatBridge);
